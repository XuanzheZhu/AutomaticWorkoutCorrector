...
var poseType = -1; // 0 -> Side Lift; 1-> Squat
const sideLiftButton = document.getElementById("sideLiftButton");
const squatButton = document.getElementById("squatButton");
const resetCountsButton = document.getElementById("resetCountsButton")
...
function drawPoses(personOrPersonPartSegmentation, flipHorizontally, ctx) {
  if (Array.isArray(personOrPersonPartSegmentation)) {
    personOrPersonPartSegmentation.forEach(personSegmentation => {
      ...
      checkPoses(pose.keypoints, angles, poseType);
    });
  } else {
